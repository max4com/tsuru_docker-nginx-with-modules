...
ARG owasp_modsecurity_crs_version
RUN set -x \
    && if [ -n "${owasp_modsecurity_crs_version}" ]; then \
        set -x \
...
ARG openresty_package_version
RUN set -x \
    && if [ -n "${openresty_package_version}" ]; then \
        set -x \
...
ARG pagespeed_ngx_version
RUN set -x \
    && if [ -n "${pagespeed_ngx_version}" ]; then \
        set -x \
